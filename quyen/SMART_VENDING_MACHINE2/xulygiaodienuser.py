...
class Ui(QtWidgets.QMainWindow):
    ...
    def add_to_cart(self):
        ...
        for selected_frame in self.selected_frames:
            ...
            if button and hasattr(button, "product_data"):
                ...
                if not self.cart.add_product(product.product_id):
                    QMessageBox.warning(self, "Lỗi",
                                        f"Không thể thêm sản phẩm '{product.name}' vào giỏ hàng. Số lượng tồn kho không đủ.")
                else:
                    # Giảm số lượng tồn kho và cập nhật nút
                    product.stock -= 1
    ...
    def remove_from_cart(self):
        ...
        for item in self.cart.to_dict():  # Duyệt qua các sản phẩm trong giỏ hàng
            product_id = item.get('product_id')
            if not product_id:
                continue
            product = self.productlist.get_product_by_id(product_id)
            if product and product.name == product_name:
                ...
                quantity_to_return = item.get('quantity', 1)  # Lấy số lượng của sản phẩm trong giỏ hàng
                break
        ...
        self.cart.remove_product(product_to_remove.product_id)
        product_to_remove.stock += quantity_to_return
        self.filter_product(self.current_category)
        self.update_cart_table()
        self.update_total_price()
